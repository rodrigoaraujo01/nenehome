...
export default function FotosNovaPage() {
  ...
  useEffect(() => {
    if (!loading && !challengeId) navigate("/fotos");
  }, [loading, challengeId, navigate]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    navigate(challengeId ? `/fotos/desafios/${challengeId}` : `/fotos/${id}`);
  }
      ...
      <main className="flex-1 px-6 py-8">
        <div className="max-w-lg mx-auto">
          <div className="flex items-center gap-3 mb-6">
            <Link to={challengeId ? `/fotos/desafios/${challengeId}` : "/fotos"} className="flex items-center gap-3 text-muted hover:text-foreground transition-colors">
              <span>‹</span>
