-- Full corrected vote_on_submission:
--   • rejection threshold fixed at >= 4 votes
--   • challenge completion logic restored (was accidentally dropped by fix_rejection_threshold.sql)
...
-- Run these queries to find the right IDs, then uncomment and run the inserts.
-- ─────────────────────────────────────────────

-- 1. Find your user id and the affected challenge:
--    select id, nickname from profiles where nickname ilike '%rodrigo%';
--    select id, title, points_reward from photo_challenges;
--    select id from photo_submissions where submitter_id = '<your_user_id>' and challenge_id = '<challenge_id>';

-- 2. Insert the missing completion record:
