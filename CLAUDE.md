...
- **Tratamento especial da vencedora** (`supabase/best_photo_achievement.sql`, roda depois de `best_photo_vote.sql`/`scoring_v2.sql`/`delete_user_content.sql`): conquista `best_photo_win` (🏆, +25) concedida no settle; medalha 🏆 no `PhotoCard` em toda aba/perfil (não só na página do desafio); seção **"🏆 Melhores fotos"** no topo da Galeria (fotos com `best_photo`). Deletar a foto vencedora ou o desafio desfaz os pontos dobrados (`challenge_best_photo`) e revoga a conquista se o autor ficar sem nenhuma vencedora (as versões de `delete_photo_submission`/`delete_photo_challenge` neste arquivo superam as anteriores).
- **Descoberta**: além do nudge na Home (`🏅 … para eleger`) e do callout na aba Fotos, os desafios com votação aberta saem da lista de "Expirados" e ganham uma seção própria **"🏅 Elegendo melhor foto"** no topo (em `/fotos/desafios` e na aba Missões), com card de borda dourada e status "Votação 🏅". Predicado compartilhado `isBestVoteOpen` em `src/lib/challenges.ts` (janela aberta + 2+ aprovadas); `getChallenges` passou a trazer `approved_count`.
